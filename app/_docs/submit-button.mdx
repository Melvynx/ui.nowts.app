...
When using multiple submit buttons, you can use the `formAction` attribute:
  ...
  return (
    <form className="space-y-4">
      ...
      <div className="flex gap-2">
        ...
        <SubmitButton formAction={publish}>
          Publish
        </SubmitButton>
      </div>
    </form>
...
Extends all `Button` component props with automatic form status detection.
...
Extends all `Button` component props with manual loading control.

```tsx
interface LoadingButtonProps extends ComponentProps<typeof Button> {
  loading?: boolean;
  success?: string; // Future feature for success states
  children: React.ReactNode;
}
```
