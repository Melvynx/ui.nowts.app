...
<CommandBlock 
	npmCommand="npx nowts-ui@latest add dialog-manager" 
	pnpmCommand="pnpm dlx nowts-ui@latest add dialog-manager" 
	yarnCommand="npx nowts-ui@latest add dialog-manager" 
	bunCommand="bunx --bun nowts-ui@latest add dialog-manager" 
/>
...
```tsx
import { DialogManagerRenderer } from "@/components/dialog-manager-renderer";
...
| `action` | `InputAction` | Action with input value callback |
| `cancel?` | `DialogCancel` | Cancel button configuration |

### dialogManager.custom(config)

| Prop | Type | Description |
|------|------|-------------|
| `children` | `ReactNode` | Custom React component to render |

### Error handling

The component automatically handles errors thrown in action callbacks and displays them as toast notifications.

## Advanced Examples

### Dialog with icon and centered layout
...
The component uses shadcn/ui components and can be styled via CSS or Tailwind:

<CodeBlock contentClassName="h-fit">

```css
/* Customize dialog animations */
.dialog-content {
